...
class wifi_rx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        # Variables
        ##################################################
        self.probe_current_state = probe_current_state = -1
        self.window_size = window_size = 48
        self.variable_qtgui_label_0 = variable_qtgui_label_0 = probe_current_state
        ...
        self.current_state = blocks.probe_signal_f()
        ...
        self._variable_qtgui_label_0_tool_bar.addWidget(Qt.QLabel('Antenna State'+": "))
        ...
        ##################################################
        # Reconfigurable Antenna Control 
        ##################################################

        # antenna state GPIO pins for four state alford loop
        # usrp pins   D8    D10   D12   D14
        ant_states = [0x01, 0x04, 0x10, 0x40]
        gpio_bank = "FP0"
        
        # Adaptive pursuit config
        K = len(ant_states)
        Pmin = 0.1
        Pmax = 1-(K-1)*Pmin
        alpha = .7 
        ...
        # Initialize GPIO bank
        self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT",  0x00) # unnecessary (but just in case)
        self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "CTRL", 0x00) # Manual GPIO control
        self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "DDR",  0xFF) # set all GPIO data direction to output 

        # Initialize antenna and adaptive pursuit 
        self.ap = adaptive_pursuit.adaptive_pursuit(K, Pmin, alpha, beta)
        self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT", 0xFF)#ant_states[self.ap.current_state])

        def _probe_current_state_probe():
            while True:
                val = self.current_state.level()
                try:
                    self.set_probe_current_state(val)
                     
                    cur_snr = self.probe_current_state
                    cur_state = self.ap.run_once(cur_snr)
                    self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT", ant_states[cur_state])
                except AttributeError:
                    pass
                time.sleep(1.0 / (100))
        _probe_current_state_thread = threading.Thread(target=_probe_current_state_probe)
        _probe_current_state_thread.daemon = True
        _probe_current_state_thread.start()
        ...
        self.blocks_pdu_to_tagged_stream_0 = blocks.pdu_to_tagged_stream(blocks.float_t, 'snr')
        ...
        self.msg_connect((self.ieee802_11_frame_equalizer_0, 'symbols'), (self.blocks_pdu_to_tagged_stream_0, 'pdus'))    
        ...
        self.connect((self.blocks_pdu_to_tagged_stream_0, 0), (self.current_state, 0))    
    ...
    def get_probe_current_state(self):
        return self.probe_current_state

    def set_probe_current_state(self, probe_current_state):
        self.probe_current_state = probe_current_state
        self.set_variable_qtgui_label_0(self._variable_qtgui_label_0_formatter(self.probe_current_state))
