...
# Title: Wifi Rx
# Generated: Mon Feb 13 15:26:50 2017
##################################################
...
class wifi_rx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.variable_qtgui_label_0 = variable_qtgui_label_0 = snr
        self.sync_length = sync_length = 320
        self.samp_rate = samp_rate = 5e6
        self.lo_offset = lo_offset = 0
        self.gain = gain = 0.75
        self.freq = freq = 2484000000
        ...
        self._variable_qtgui_label_0_tool_bar.addWidget(Qt.QLabel('snr'+": "))
        ...
        self.blocks_pdu_to_tagged_stream_1_0 = blocks.pdu_to_tagged_stream(blocks.float_t, 'packet len')
        ...
        self.snr_probe = blocks.probe_signal_f()
        ...
        #ant_states = [0x01, 0x04, 0x10, 0x40] #90 degree states
        ant_states = [0x05, 0x14, 0x50, 0x41]
        gpio_bank = "FP0"
        ...
        self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT", ant_states[self.ap.current_state])
        self.cur_snr = -10
        self.prev_snr = self.cur_snr
        self.i = 0;
        
        def _snr_probe():
            while True:
                #val = self.snr_probe.level()
                self.cur_snr = self.ieee802_11_frame_equalizer_0.get_eq_snr()
                try:

                    # Run AP
                    cur_state = self.ap.run_once(self.cur_snr)

                #    if self.cur_snr == self.prev_snr:
                #        cur_state = self.ap.run_once(self.cur_snr)
                #    else:
                #        cur_state = self.ap.run_once()
                    if self.cur_snr != self.prev_snr:

                        #self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT", 0xFF)
                        self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT", ant_states[cur_state])

                        #self.uhd_usrp_source_0.set_gpio_attr(gpio_bank, "OUT", ant_states[self.i])
                        self.i = (self.i+1) % len(ant_states)

                        ### set label ###
                        self.set_snr(self.cur_snr)
                        #self.set_snr(self.ap.optimal_arm)
        ...
        self.msg_connect((self.ieee802_11_frame_equalizer_0, 'symbols'), (self.blocks_pdu_to_tagged_stream_1_0, 'pdus'))    
        ...
        self.connect((self.blocks_pdu_to_tagged_stream_1_0, 0), (self.snr_probe, 0))    
    ...
    def set_snr(self, snr):
        self.snr = snr
        self.set_variable_qtgui_label_0(self._variable_qtgui_label_0_formatter(self.snr))
